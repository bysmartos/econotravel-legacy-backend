...
const experienciasController = {
  ...
  postExperiencia: async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        const result = await experienciasModel.postExperiencia({
          titulo,
          imagen,
          descripcion,
          precio,
  ...
  updateExperiencia: async (req: Request, res: Response) => {
    try {
      ...
      const result = await experienciasModel.updateExperiencia(
        {
          titulo,
          imagen,
          descripcion,
  ...
  deleteExperiencia: async (req: Request, res: Response) => {
    try {
      ...
      result
        ? res
            .status(201)
            .json({ result: `Experiencia deleted with ID: ${param}` })
  ...
  postExperienciaFilter: async (req: Request, res: Response) => {
    try {
      ...
      const result = await experienciasModel.postExperienciaFilter({
        searcher,
      });
      result
        ? res.status(201).json(result)
